            ...
            for k = 1:num_iterations
                for j = 1:length(obj.swarms)
                    for m = 1:length(obj.swarms(j).particles)
                        waitbar(count/(num_iterations*length(obj.swarms)*length(obj.swarms(j).particles)));
                        code = strcat('load("',char(obj.file_name),'");',...
                            'switchtolayout;');
                        appevalscript(h, code);
                        
                        % update structure parameters
                        for n = 1:length(obj.swarms(j).particles(m).inputs)
                            code = strcat('select("',...
                                char(obj.swarms(j).particles(m).inputs(n).structure),'");',...
                                'set("',char(...
                                obj.swarms(j).particles(m).inputs(n).parameter),...
                                '", ',num2str(obj.swarms(j).particles(m).position(n)),');');
                            appevalscript(h, code);
                        end
                        ...
                        % update global bests
                        if obj.swarms(j).particles(m).FOM >= obj.swarms(j).best_FOM
                            obj.swarms(j).best_FOM = obj.swarms(j).particles(m).FOM;
                            obj.swarms(j).best_position = obj.swarms(j).particles(m).position;
                            obj.swarms(j).stag_FOM = 0;
                        end
                        
                        % update particle bests
                        if obj.swarms(j).particles(m).FOM >= obj.swarms(j).particles(m).best_FOM
                            obj.swarms(j).particles(m).best_FOM = obj.swarms(j).particles(m).FOM;
                        ...
                        if plot_data == true
                            ...
                            ylim(obj.swarms(j).particles(1).inputs(2).range);
                            xlim(obj.swarms(j).particles(1).inputs(1).range);
                            hold on;
                        end
                        
                        % update velocity and position of particle
                        obj.swarms(j).particles(m).velocity...
                            = obj.swarms(j).intertia*obj.swarms(j).particles(m).velocity...
                            + obj.swarms(j).c1*rand*(obj.swarms(j).particles(m).best_position...
                            - obj.swarms(j).particles(m).position)...
                            + obj.swarms(j).c2*rand*(obj.swarms(j).best_position...
                            - obj.swarms(j).particles(m).position);
                        obj.swarms(j).particles(m).position...
                            = obj.swarms(j).particles(m).position + obj.swarms(j).particles(m).velocity;
                        ...
                        for n = 1:length(obj.swarms(j).particles(m).inputs)
                            if obj.swarms(j).particles(m).position(n)...
                                    > obj.swarms(j).particles(m).inputs(n).range(2)
                                obj.swarms(j).particles(m).position(n)...
                                    = obj.swarms(j).particles(m).inputs(n).range(2);
                            elseif obj.swarms(j).particles(m).position(n)...
                                    < obj.swarms(j).particles(m).inputs(n).range(1)
                                obj.swarms(j).particles(m).position(n)...
                                    = obj.swarms(j).particles(m).inputs(n).range(1);
                            end
                        end

classdef Data < handle
    ...
    methods
        ...
        function labels = get_labels_2(obj, features, r, h)
            ...
            code = strcat('load("H:/MDM PW/ring_wrapped_TE2.fsp");',...
                'switchtolayout;');
            appevalscript(r, code);
            
            for n = 1:length(obj.inputs)
                code = strcat('select("w1");',...
...
%             end
            ...
            % RUN!
            code = strcat('runsweep("s-parameter sweep");',...
                'exportsweep("s-parameter sweep","ring_wrapped_TE2_TE2IN.dat");');
            appevalscript(r, code);
            
            labels = [];
            code = strcat('load("H:/MDM PW/ring_wrapped_TE2.icp");',...
                'switchtolayout;',...
                'run;',...
                'a = getresult("TE2","input 1/mode 1/gain");',...
                'fom = min(a.getattribute("mode 3 gain (dB)"));');
            appevalscript(h, code);
            labels = cat(2, labels, appgetvar(h, 'fom')');
        end
